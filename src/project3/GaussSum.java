...
public class GaussSum {
    public static int gaussSumOf(int a) {
        int total = 0;
        for (int i = a; i > 0; i--) {
            total = total + i;
        }
    ...
    public static int sumOfSquaresOfGaussSums(int a, int b) {
        int aSquare = gaussSumOf(a)*gaussSumOf(a);
        int bSquare = gaussSumOf(b)*gaussSumOf(b);
